...
API, the learner can: score a new document based on the current model state; 
update the model state when given a document and a positive or negative class 
label; report its current state as a string; or reset itself to it's original, zero-observations
state.

\begin{figure}[h]
\includegraphics[width=0.8\textwidth]{fig/tex/single_txt_cls.png}
\centering
...
\begin{figure}[h]
\includegraphics[width=0.8\textwidth]{fig/tex/double_txt_cls.png}
\centering
...
new requests). Note how the order requests are completed may no longer match 
...
%tk tk the political/managerial aspects
